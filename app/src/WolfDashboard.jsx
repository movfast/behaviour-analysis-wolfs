    ...
    >
      <span
        style={{
          ...
          width: 18, height: 18, borderRadius: "50%", background: "rgba(96,165,250,0.18)",
          border: "1px solid rgba(96,165,250,0.45)", color: "#60a5fa", fontSize: 11,
          fontWeight: 700, cursor: "help", userSelect: "none", lineHeight: 1,
        }}
      >
        ?
      </span>
      {visible && (
        <span
          style={{
            ...
            color: "#cbd5e1", fontSize: 12, lineHeight: 1.55, width: 260, zIndex: 999,
...
const IntroBlock = ({ children }) => (
  <div style={{
    background: "rgba(96,165,250,0.06)", borderLeft: "3px solid rgba(96,165,250,0.4)",
    borderRadius: "0 10px 10px 0", padding: "14px 18px", marginBottom: 20,
...
function ConfusionMatrix() {
  ...
  return (
    <div>
      <SectionTitle sub="Random Forest on held-out test set (720 samples)"><span>Confusion Matrix <InfoTooltip text="A confusion matrix shows where the model gets it right and where it gets confused. The diagonal (green) cells are correct predictions. Off-diagonal (red) cells are mistakes — for example, a Foraging wolf being misclassified as Resting." /></span></SectionTitle>
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:"#0a0f1e",minHeight:"100vh",padding:"24px 20px",color:"#f1f5f9"}}>
      ...
      {/* What is this? */}
      <div style={{background:"rgba(96,165,250,0.08)",border:"1px solid rgba(96,165,250,0.2)",borderRadius:12,padding:"16px 20px",marginBottom:20,maxWidth:640,margin:"0 auto 24px"}}>
        <div style={{color:"#60a5fa",fontWeight:700,fontSize:14,marginBottom:6}}>What is this?</div>
        <p style={{color:"#94a3b8",fontSize:13,lineHeight:1.6,margin:0}}>
          This dashboard shows how scientists track wolves using GPS collars and use machine learning to automatically figure out what the wolves are doing — resting, looking for food (foraging), or traveling. Explore the tabs below to go from a high-level summary to the technical details.
        </p>
      </div>
      ...
      {tab==="overview" && <div style={{display:"flex",flexDirection:"column",gap:20}}>
        <IntroBlock>
          Welcome! This tab gives you the big picture. You will see how much data was collected, how accurately our computer models can predict wolf behavior, and how the different behaviors break down across the dataset. No prior knowledge needed — just explore!
        </IntroBlock>
        <div style={{display:"flex",gap:14,flexWrap:"wrap"}}>
          <StatCard label={<span>GPS Fixes <InfoTooltip text="A GPS fix is a single location reading from a satellite collar attached to a wolf. Every 30 minutes the collar records where the wolf is, giving scientists a trail of dots to analyze." /></span>} value="3,600" sub="6 wolves × 600 steps" color="#60a5fa"/>
          <StatCard label={<span>RF Accuracy <InfoTooltip text="Random Forest is a machine learning method that builds many decision trees (like a flowchart) and combines their votes to make a prediction. 99% accuracy means it almost always guesses the wolf's behavior correctly." /></span>} value="99.0%" sub={<span>Cross-val: 97.6% <InfoTooltip text="Cross-validation is a technique where the model is tested on data it has never seen before, multiple times, to make sure the accuracy is not a fluke." /></span>} color="#34d399"/>
          <StatCard label={<span>GB Accuracy <InfoTooltip text="Gradient Boosting is another machine learning method. It builds trees one at a time, with each new tree trying to fix the mistakes of the previous ones. Think of it as learning from errors." /></span>} value="98.9%" sub={<span>Cross-val: 98.2% <InfoTooltip text="Cross-validation is a technique where the model is tested on data it has never seen before, multiple times, to make sure the accuracy is not a fluke." /></span>} color="#a78bfa"/>
          <StatCard label={<span>Behaviors <InfoTooltip text="Scientists classify wolf activity into 3 states: Resting (sleeping or inactive), Foraging (searching for food), and Traveling (moving purposefully between locations). These cover the main things wolves do." /></span>} value="3" sub="Resting · Foraging · Traveling" color="#f97316"/>
        </div>

        <Section>
          <SectionTitle sub="Distribution of behavioral states across all 6 wolves"><span>Behavior Distribution <InfoTooltip text="Distribution means how the total data is divided up. Here it shows what percentage of the time wolves were resting vs. foraging vs. traveling across all observations." /></span></SectionTitle>
      ...
      {/* TRACKS TAB */}
      {tab==="tracks" && <div style={{display:"flex",flexDirection:"column",gap:20}}>
        <IntroBlock>
          This tab shows where each wolf went. Every dot on the map is one GPS reading from a wolf's collar, taken every 30 minutes. The color of the dot tells you what the wolf was doing at that moment. Notice how resting dots cluster tightly (the wolf stays put) while traveling dots spread out across the landscape.
          <InfoTooltip text="Each dot represents a single GPS fix — a snapshot of where the wolf was at a specific moment. Tracking movement helps scientists understand how wolves use their territory, find food, and interact with their environment." />
        </IntroBlock>
        <Section>
          <TrackMap/>
            ...
            {["Resting","Foraging","Traveling"].map(b=>(
              <div key={b} style={{background:"rgba(255,255,255,0.03)",borderRadius:10,padding:14,border:`1px solid ${COLORS[b]}33`}}>
                ...
                <div style={{color:"#94a3b8",fontSize:12,lineHeight:1.6}}>
                  <div>⚡ Speed: <b style={{color:"#e2e8f0"}}>{behaviorStats[b].speed} km/h</b> <InfoTooltip text="Speed is how fast the wolf was moving between two GPS readings, measured in kilometers per hour. Resting wolves barely move; traveling wolves can cover significant ground." /></div>
                  <div>📏 Step: <b style={{color:"#e2e8f0"}}>{behaviorStats[b].step} km</b> <InfoTooltip text="Step length is the straight-line distance between two consecutive GPS fixes. Short steps mean the wolf stayed nearby; long steps mean it moved far." /></div>
                  <div>🔄 Turning: <b style={{color:"#e2e8f0"}}>{behaviorStats[b].turning} rad</b> <InfoTooltip text="Turning angle measures how much the wolf changed direction between steps, in radians. High turning = zigzagging (foraging). Low turning = moving in a straight line (traveling)." /></div>
      ...
      {tab==="features" && <div style={{display:"flex",flexDirection:"column",gap:20}}>
        <IntroBlock>
          Features are the measurable quantities scientists extract from raw GPS data to feed into the machine learning models. Think of them as clues — speed, step length, and turning angle each tell the model something different about what the wolf is doing. This tab shows how those clues differ between behaviors.
        </IntroBlock>
        <Section>
      ...
      {tab==="model" && <div style={{display:"flex",flexDirection:"column",gap:20}}>
        <IntroBlock>
          This tab dives into the machine learning models. Two algorithms were trained to classify wolf behavior: Random Forest and Gradient Boosting. You will see how accurate each model is, where it makes mistakes (confusion matrix), and which measurements matter most for its predictions (feature importance). Precision, Recall, and F1 scores measure different aspects of how well the model performs for each behavior.
        </IntroBlock>
        <div style={{display:"flex",gap:14,flexWrap:"wrap"}}>
          {[
            ...
            <Section key={model} style={{flex:1,minWidth:200}}>
              ...
              <div style={{display:"flex",gap:16,marginBottom:14}}>
                ...
                <div><div style={{color:"#94a3b8",fontSize:11}}>CV Accuracy <InfoTooltip text="Cross-validation (CV) accuracy is measured by training and testing the model multiple times on different slices of the data. It gives a more reliable estimate of how well the model will work on new, unseen data." /></div><div style={{color:"#f1f5f9",fontSize:22,fontWeight:700}}>{cv}</div></div>
              </div>
              {Object.entries(metrics).map(([cls,m])=>(
                <div key={cls} style={{marginBottom:8}}>
                  ...
                  <div style={{display:"flex",gap:1}}>
                    {[["P",m.p,"Precision: of all the times the model predicted this behavior, how often was it correct?"],["R",m.r,"Recall: of all the actual instances of this behavior, how many did the model successfully find?"],["F1",m.f1,"F1 Score: a combined measure of Precision and Recall. A score of 1.0 is perfect."]].map(([lbl,val,tip])=>(
                      <div key={lbl} style={{flex:1,background:`rgba(255,255,255,0.05)`,borderRadius:4,padding:"3px 6px",textAlign:"center"}}>
                        <div style={{color:"#475569",fontSize:9}}>{lbl} <InfoTooltip text={tip} /></div>
        ...
        <Section>
          <SectionTitle sub="Which features matter most for predicting behavior? (Random Forest)"><span>Feature Importance <InfoTooltip text="Feature importance shows how much each measurement contributes to the model's predictions. Higher importance means the model relies more heavily on that feature to tell behaviors apart." /></span></SectionTitle>
          <ResponsiveContainer width="100%" height={260}>
      ...
      {tab==="findings" && <div style={{display:"flex",flexDirection:"column",gap:16}}>
        <IntroBlock>
          This is where we put it all together. These findings summarize what we learned from the data and models. Each card highlights a key takeaway — from how well the models performed, to which measurements matter most, to what this kind of research means for real-world wolf conservation.
        </IntroBlock>
        <Section style={{borderLeft:"3px solid #34d399",paddingLeft:20}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            <span style={{fontSize:20}}>🎯</span>
            <h3 style={{color:"#34d399",margin:0,fontSize:16,fontWeight:700}}>Near-Perfect Classification <InfoTooltip text="Classification means sorting data into categories. Here, the model sorts each GPS reading into one of three behavior categories. 'Near-perfect' means it gets the right answer over 99% of the time." /></h3>
          </div>
          <p style={{color:"#94a3b8",fontSize:13,lineHeight:1.65,margin:0}}>Both Random Forest (99.0%) and Gradient Boosting (98.9%) achieved exceptional accuracy. Traveling was classified with 100% precision and recall — its extreme speed values make it trivially separable from the other states.</p>
        </Section>
        <Section style={{borderLeft:"3px solid #60a5fa",paddingLeft:20}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            <span style={{fontSize:20}}>⚡</span>
            <h3 style={{color:"#60a5fa",margin:0,fontSize:16,fontWeight:700}}>Speed Is King <InfoTooltip text="'Orders of magnitude' means the values differ by factors of 10. Here, a resting wolf moves 100x slower than a traveling wolf, making speed an obvious giveaway for behavior." /></h3>
          </div>
          <p style={{color:"#94a3b8",fontSize:13,lineHeight:1.65,margin:0}}>Speed and log-speed together account for 58.5% of total feature importance. The speed difference between behavioral states spans 3 orders of magnitude: Resting wolves move ~0.01 km/h, Foraging ~0.23 km/h, and Traveling ~1.7 km/h.</p>
        </Section>
        <Section style={{borderLeft:"3px solid #f97316",paddingLeft:20}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            <span style={{fontSize:20}}>🔄</span>
            <h3 style={{color:"#f97316",margin:0,fontSize:16,fontWeight:700}}>Turning Angle Separates Foraging from Traveling <InfoTooltip text="Area-restricted search means the wolf is systematically searching a small area for prey, zigzagging back and forth. Directional movement means the wolf knows where it's going and moves in a relatively straight line." /></h3>
          </div>
          <p style={{color:"#94a3b8",fontSize:13,lineHeight:1.65,margin:0}}>The most interesting distinction: Foraging wolves show high turning angles (1.12 rad average) due to area-restricted search behavior, while Traveling wolves move directionally (0.46 rad). This is the key feature for discriminating these two active states.</p>
        </Section>
        <Section style={{borderLeft:"3px solid #a78bfa",paddingLeft:20}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            <span style={{fontSize:20}}>🌙</span>
            <h3 style={{color:"#a78bfa",margin:0,fontSize:16,fontWeight:700}}>Time of Day Has Minimal Effect <InfoTooltip text="Diel patterns are daily activity cycles — like how humans sleep at night and work during the day. Some animals are strictly nocturnal or diurnal, but wolves appear active at all hours." /></h3>
          </div>
          <p style={{color:"#94a3b8",fontSize:13,lineHeight:1.65,margin:0}}>Surprisingly, time-of-day features (sin/cos of hour, is_night) collectively contribute less than 3% of importance. Wolves in this simulation are active across all hours — behavioral state is driven by locomotion metrics, not diel patterns.</p>
        </Section>
        <Section style={{borderLeft:"3px solid #fbbf24",paddingLeft:20}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            <span style={{fontSize:20}}>🐺</span>
            <h3 style={{color:"#fbbf24",margin:0,fontSize:16,fontWeight:700}}>What This Enables in Real Research <InfoTooltip text="Traditionally, researchers had to physically observe wolves to know what they were doing — expensive and limited. With ML classifiers on GPS data, they can study thousands of wolves simultaneously from their laptops." /></h3>
          </div>
          <p style={{color:"#94a3b8",fontSize:13,lineHeight:1.65,margin:0}}>With a trained classifier like this, ecologists can automatically label millions of GPS fixes from deployed collars, enabling population-scale studies of habitat use, energy budgets, predator-prey dynamics, and human disturbance effects — all without costly direct observation.</p>
        </Section>
        <Section style={{borderLeft:"3px solid #fb7185",paddingLeft:20}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            <span style={{fontSize:20}}>🚀</span>
            <h3 style={{color:"#fb7185",margin:0,fontSize:16,fontWeight:700}}>Next Steps <InfoTooltip text="ODBA (Overall Dynamic Body Acceleration) measures actual body movement from an accelerometer on the collar. HMMs (Hidden Markov Models) consider the sequence of behaviors over time rather than classifying each point independently. Transfer learning tests whether a model trained on one group of wolves works for a different group." /></h3>
          </div>
